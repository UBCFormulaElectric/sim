...
bool Triangulation::on_edge(const Cone& cone, const size_t a, const size_t b) const {
    ...
    if (const double cross_product = (cone.y - cone_a.y) * (cone_b.x - cone_a.x) - (cone.x - cone_a.x) * (cone_b.y - cone_a.y);
...
void Triangulation::insert(const Cone& cone, const size_t cone_id) {
    ...
    const std::shared_ptr<TrianglePyramid> t = root.find_triangle(cone, existing_cones);
